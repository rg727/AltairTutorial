...
IT = np.loadtxt('Intertemporal.resultfile')
...
def makeFigure6():
    ...
    for i in range(len(lake_state)):
        Y1[i] = DPSpolicy(lake_state[i], DPS[1,0:6]) # DPS best reliability
        Y2[i] = DPSpolicy(lake_state[i], DPS[np.argmin(DPS[:,6]),0:6]) # DPS best benefits
    
    
    lake_state = np.arange(0,2.5,0.01)    
    Y=np.zeros([len(DPS),len(lake_state)]) 
    for z in range(len(DPS)):
        for i in range(len(lake_state)):
            Y[z,i] = DPSpolicy(lake_state[i], DPS_resultfile[z,0:6]) # DPS best reliability
